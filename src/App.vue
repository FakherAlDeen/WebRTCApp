...
import Modal from "../src/components/ModalInfo.vue"
...
watch(()=>UserStore().Notify.length, (first,second)=>{
  ...
  if(UserStore().pageImIn !='List' && UserStore().isLoggedIN){
...
console.log('rounter', useRoute().path);
console.log(UserStore().pageImIn);
const showNavBar= ref(true);
UserStore().navBarOn = showNavBar.value
// console.log(UserStore().Notifications);
let imgsrc = "../src/assets/svgs/left-arrow-svgrepo-com.svg"; 
 function ClickShowHide (){
  showNavBar.value = !showNavBar.value
  UserStore().navBarOn = showNavBar.value
  console.log(UserStore().navBarOn);
}
const showModal = ref (false);
const ModalHeader = ref('z');
const ModalText = ref('z');
async function ClickMeeting(){
...
<template class="box-border">
  ...
  <button class="btn btn-circle btn-ghost fixed z-40 top-16 left-16" v-if ='UserStore().isLoggedIN'  @click="ClickShowHide">
    <img v-if="showNavBar" class="w-6 mr-1" src='../src/assets/svgs/left-arrow-svgrepo-com.svg'/> 
    <img v-else class="w-6 " src='../src/assets/svgs/right-arrow-svgrepo-com.svg'/> 
  </button>
  <div class="rounded-3xl overflow-hidden bg-gray-50 h-full">
    <img class = "absolute top-[13rem] w-full h-full left-0 overflow-hidden scale-150" src='../src/assets/svgs/waihou.svg'/>
    <!-- <div class = "absolute top-0 w-full h-full left-0 overflow-hidden bg-[url('../src/waihou.svg')] bg-cover" ></div> -->
    <div class="flex h-full">
      <div class="absolute top-[3.5rem] right-10 flex flex-row-reverse z-10" v-if = "UserStore().isLoggedIN== true">
        <div class="dropdown dropdown-end z-10">
          <label tabindex="0" class="btn btn-ghost btn-circle">
            <div class="indicator">
              ...
              <span v-if = "UserStore().RequestList.find(v=>v.type=='gotfrom')" class="badge badge-sm badge-info indicator-item"></span>
            </div>
          </label>
          <div tabindex="0" class="z-10 mt-3 card card-compact dropdown-content w-fit bg-base-100 shadow">
            <div class="card-body space-y-1">
              <span class="font-bold text-lg">Your Friend Requests:</span>
              <div class="divider mt-0"></div>
                <div class="w-80 max-h-[20rem] overflow-y-auto gap-2">
                  <template v-if="!UserStore().IsAGuest">
                    <template  v-for="val in UserStore().RequestList" :key="val">
                      <div class="card-body mb-2 card shadow-md bg-primary text-primary-content " v-if="val.type == 'gotfrom'">
                        <p class="font-medium text-sm"> <span class="font-medium text-lg">{{val.username}}</span> Added you!</p>
                        <div class="flex w-full flex-wrap">
                          <button class="btn btn-md m-1 btn-success w-fit grow" @click="UserStore().AcceptReq(val.id)">Accept</button>
                          <button class="btn btn-md m-1 btn-error w-fit grow" @click="UserStore().RejectReq(val.id)">Reject</button>
                        </div>
                      </div>
                    </template>
                  </template>
                </div> 
                <div v-if= "UserStore().IsAGuest || !UserStore().RequestList.find(v=>v.type=='gotfrom')">
                  <img src="../src/assets/svgs/ill1.svg"/>
                  <p class="font-base text-lg p-5 text-center">There are no Requests sent to you!</p>
                </div>
            </div>
          </div>
        </div>
      </div>
      <div class="absolute top-[3.5rem] right-[5.5rem] flex flex-row-reverse z-10" v-if = "UserStore().isLoggedIN== true">
        <div class="dropdown dropdown-end z-10">
          <label tabindex="0" class="btn btn-ghost btn-circle">
            <div class="indicator">
              <img class="w-6" src='../src/assets/svgs/NotificationIcon.svg'/>
              <span v-if = "UserStore().Notifications.length>0" class="badge badge-sm badge-info indicator-item text-white">{{UserStore().Notifications.length}}</span>
            </div>
          </label>
          <div tabindex="0" class="z-10 mt-3 card card-compact dropdown-content w-fit bg-base-100 shadow">
            <div class="card-body space-y-1">
              <span class="font-bold text-lg">Your Notifications History:</span>
              <div class="divider mt-0"></div>
              <div class="card-actions">
                <div class="w-80 max-h-[20rem] overflow-y-auto">
                  <template v-if="!UserStore().IsAGuest && UserStore().Notifications.length>0">
                    <template  v-for="val in UserStore().Notifications" :key="val">
                      <div class="card-body mb-2 card shadow-md bg-primary text-primary-content " v-if="val.type == 'invite'">
                        <p class="font-medium text-sm"> <span class="font-medium text-lg">{{val.username}}</span> invited you to A meeting</p>
                        <div class="flex w-full flex-wrap">
                          <button class="btn btn-md m-1 btn-success w-fit grow" @click="UserStore().RejoinMeeting(val.SharingID,val.MeetingName)">Rejoin</button>
                        </div>
                        <p class="font-base text-sm">Before {{parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000)}}H and {{parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /60000) - 60 * parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000)}}M</p>
                      </div>
                      <div class="card-body mb-2 card shadow-md bg-primary text-primary-content " v-else>
                        <p class="font-medium text-sm"> <span class="font-medium text-lg">{{val.username}}</span> You invited {{val.username}} and he joined</p>
                        <div class="flex w-full flex-wrap">
                          <button class="btn btn-md m-1 btn-success w-fit grow" @click="UserStore().RejoinMeeting(val.SharingID,val.MeetingName)">Rejoin</button>
                        </div>
                        <p v-if = "parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000) || parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /60000) - 60 * parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000)" class="font-base text-sm">Before {{parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000)}}H and {{parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /60000) - 60 * parseInt(Math.abs(new Date(Date.now()).getTime() - (new Date(val.date)).getTime()) /3600000)}}M</p>
                        <p v-else>Now</p>
                      </div>
                    </template>
                  </template>
                </div> 
                <div v-if= "UserStore().IsAGuest || UserStore().Notifications.length == 0">
                  <img src="../src/assets/svgs/ill1.svg"/>
                  <p class="font-base text-lg p-5 text-center">There are no Notifications for you!</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="w-1/5 min-w-[20%] bg-white m-5 rounded-3xl drop-shadow-xl flex flex-col" v-show="showNavBar">
        <template v-if="UserStore().pageImIn == 'Login' || UserStore().pageImIn == 'Signup'">
          <router-view ></router-view>
        </template>
        <template v-else>
          <div class="avatar placeholder">
          <div class="bg-neutral-focus text-neutral-content rounded-full w-1/3 mx-auto mt-20">
            <span v-if="UserStore().userDetails" class="text-3xl">{{UserStore().userDetails.username.slice(0,2).toUpperCase()}}</span>
          </div>
          </div>
          <p class="text-lg text-gray-500 font-light mt-4 text-center">Welcome Back</p>
          <h3 class="text-2xl text-gray-700 font-bold text-center">{{UserStore().userDetails.Fname + " " + UserStore().userDetails.Lname}}</h3>
          <button @click="router.push('/list')" class="btn btn-ghost m-5 my-2 mt-32" :class="{'btn-active':UserStore().pageImIn == 'List'}" >
            <span class="mr-2">
              <svg width="24px" height="24px"  viewBox="0 0 24 24" fill="#242424" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M14.2929 4.29289C14 4.58579 14 5.05719 14 6C14 6.94281 14 7.41421 14.2929 7.70711C14.5858 8 15.0572 8 16 8H18C18.9428 8 19.4142 8 19.7071 7.70711C20 7.41421 20 6.94281 20 6C20 5.05719 20 4.58579 19.7071 4.29289C19.4142 4 18.9428 4 18 4H16C15.0572 4 14.5858 4 14.2929 4.29289ZM4.29289 16.2929C4 16.5858 4 17.0572 4 18C4 18.9428 4 19.4142 4.29289 19.7071C4.58579 20 5.05719 20 6 20H8C8.94281 20 9.41421 20 9.70711 19.7071C10 19.4142 10 18.9428 10 18C10 17.0572 10 16.5858 9.70711 16.2929C9.41421 16 8.94281 16 8 16H6C5.05719 16 4.58579 16 4.29289 16.2929ZM14 14C14 13.0572 14 12.5858 14.2929 12.2929C14.5858 12 15.0572 12 16 12H18C18.9428 12 19.4142 12 19.7071 12.2929C20 12.5858 20 13.0572 20 14V18C20 18.9428 20 19.4142 19.7071 19.7071C19.4142 20 18.9428 20 18 20H16C15.0572 20 14.5858 20 14.2929 19.7071C14 19.4142 14 18.9428 14 18V14ZM4.29289 4.29289C4 4.58579 4 5.05719 4 6V10C4 10.9428 4 11.4142 4.29289 11.7071C4.58579 12 5.05719 12 6 12H8C8.94281 12 9.41421 12 9.70711 11.7071C10 11.4142 10 10.9428 10 10V6C10 5.05719 10 4.58579 9.70711 4.29289C9.41421 4 8.94281 4 8 4H6C5.05719 4 4.58579 4 4.29289 4.29289Z" fill="black"/>
          ...
          <button class="btn btn-ghost m-5 my-2" @click="ClickMeeting" :class="{'btn-active':UserStore().pageImIn == 'Meeting'}" >
            <span class="mr-2">
              <img class="w-6" src="../src/assets/svgs/meeting-svgrepo-com.svg"/>
            </span>
            <span>
              Meeting
            </span>
          </button>
          <button class="btn btn-ghost m-5 my-2 mb-auto" @click="ClickDoc" :class="{'btn-active':UserStore().pageImIn == 'Document'}" >
            <span class="mr-2">
              <img class="w-5" src="../src/assets/svgs/document-svgrepo-com.svg"/>
            </span>
            <span>
              Document
            </span>
          </button>
          <button class="btn btn-ghost m-5 my-2 mb-1" :class="{'btn-active':UserStore().pageImIn == 'profile'}" @click="router.push('/profileEdit')">
            <span class="mr-2">
              <img class="w-5" src="../src/assets/svgs/profile-svgrepo-com.svg"/>
            </span>
            <span>
              Profile
            </span>
          ...
          <button class="btn btn-ghost m-5 mb-6 " @click="UserStore().SignOut()" >
            <span class="mr-2">
      ...
      <template v-if="UserStore().pageImIn != 'Login' && UserStore().pageImIn != 'Signup'">
        <Modal :open="showModal" :Header="ModalHeader" :txt="ModalText" @on-close="showModal = false"/>
        <div class="bg-white m-5 rounded-3xl drop-shadow-xl overflow-hidden box-border" :class="{'w-4/5 ml-0':UserStore().navBarOn , 'w-full':!UserStore().navBarOn}">
          <router-view ></router-view>
        </div>
      </template>
    </div>
  </div>
  <Modal @onClose="openInviteModal = false" :usernameInvite="usernameInvite" :Header="Gotinvitedtxt" txt="Check your notification bar to join later!"  :withBtn1="false" :withBtn2="false" FuncBtn1="justClose" FuncBtn2="AcceptInivte" :open="openInviteModal"/>
  <ReloadPrompt />
</template>